...
function getReservaciones(){
    ...
    $.ajax({
        url:"http://192.9.147.170:8080/api/Reservation/all",
        type:"GET",
        datatype:"JSON",
...
function postReservaciones(){
    ...
    alert("Todos los campos son obligatorios");
        ...
        $.ajax({
            url:"http://192.9.147.170:8080/api/Reservation/save",
            type:"POST",
            datatype:"JSON",
...
function putReservaciones(idBotonActualizar){
    ...
    alert("Todos los campos son obligatorios");
        ...
        $.ajax({
            url:"http://192.9.147.170:8080/api/Reservation/update",
            type:"PUT",
            datatype:"JSON",
            contentType:"application/json; charset=utf-8",
            data: JSON.stringify(cajas),
...
function deleteReservaciones(idBotonBorrar){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url:"http://192.9.147.170:8080/api/Reservation/"+ idBotonBorrar,
                type:"DELETE",
                datatype:"JSON",
...
function pintarReservation(respuesta){
    ...
    for(i=0;i<respuesta.length;i++){
        ...
        myTable+="<td> <button onclick='putReservaciones("+respuesta[i].idReservation+")'class='flex mx-auto text-white bg-cyan-900 border-0 py-2 px-8 focus:outline-none hover:bg-cyan-600 rounded text-lg'>Actualizar</button> "
        myTable+="<td> <button onclick='deleteReservaciones("+respuesta[i].idReservation+")'class='flex mx-auto text-white bg-cyan-900 border-0 py-2 px-8 focus:outline-none hover:bg-cyan-600 rounded text-lg'>Borrar</button> "
        myTable+="</tr>";   
    }
    myTable+="</table>";
